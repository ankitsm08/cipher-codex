...
string toBinary(unsigned char num, const char& c0, const char& c1) {
  string bin;
  bin.reserve(binaryLength);

  while (num > 0) {
    bin = ((num % 2 == 0) ? c0 : c1) + bin;
    num /= 2;
  }
  // Ensure the binary string is 5 characters
  while (bin.length() < 5) {
    bin = c0 + bin;
  }
...
string bacon::encrypt(const string& text, char c0, char c1) {
  string code = "";
  code.reserve(text.size() * binaryLength);
...
string bacon::decrypt(const string& code, char c0, char c1) {
  string text = "";
  text.reserve(code.size() / binaryLength);
  c0 = toupper(c0); c1 = toupper(c1);

  for (int i = 0; i < code.length();) {
    if (code[i] == c0 || code[i] == c1) {
      text += 'A' + toCharacter(code.substr(i, binaryLength), c1);
      i += 5; continue;
